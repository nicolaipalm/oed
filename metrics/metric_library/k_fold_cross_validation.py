...
class KFoldCrossValidation(Metric):
    ...
    def calculate(self,
        ...
        for train_index, test_index in k_fold.split(evaluations_blackbox_function):
            design_training, design_test = design.design[train_index], design.design[test_index]
            y_training, y_test = evaluations_blackbox_function[train_index], evaluations_blackbox_function[test_index]
            estimated_parameter = self._statistical_model.calculate_maximum_likelihood_estimation(x0=design_training,
                                                                                                  y=y_training,
                                                                                                  minimizer=self._minimizer)

            error.append(self._error_function(y1=y_test,
                                              y2=np.array(
                                                  [self._statistical_model(theta=estimated_parameter, x=x) for
                                                   x in design_test])))
