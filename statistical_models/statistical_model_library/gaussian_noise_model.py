...
class GaussianNoiseModel(StatisticalModel):
    ...
    def calculate_fisher_information(self, theta: np.ndarray, i: int, j: int, x0: np.ndarray):
        return 1 / self._sigma * np.dot(
            np.array([
                self._function.partial_derivative(
                    theta=theta, x=x_k, parameter_index=i) for x_k in x0
            ]).T,
            np.array([
                self._function.partial_derivative(
                    theta=theta, x=x_k, parameter_index=j) for x_k in x0
            ]))
    ...
    def calculate_log_likelihood(self, x0: np.ndarray, y: np.ndarray, theta: np.ndarray) -> np.ndarray:
        return -np.sum(
            (y - np.array([self._function(theta=theta, x=x) for x in x0])) ** 2)

    def calculate_partial_derivative_log_likelihood(self, x0: np.ndarray, y: np.ndarray, theta: np.ndarray,
                                                    parameter_index: int) -> np.ndarray:
        return - np.sum(
            (y - np.array([self._function(theta=theta, x=x0) for x in x0])) *
            np.array([
                self._function.partial_derivative(
    ...
    def calculate_maximum_likelihood_estimation(self,
                                                x0: np.ndarray,
                                                y: np.ndarray,
                                                minimizer: Minimizer) -> np.ndarray:
        return minimizer(function=lambda theta: -self.calculate_log_likelihood(theta=theta, y=y, x0=x0),
