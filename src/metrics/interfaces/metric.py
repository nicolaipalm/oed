...
from src.visualization.plotting_functions import dot_scatter, styled_figure, line_scatter
...
class Metric(ABC):
    ...
    def plot(self,
             evaluations_blackbox_function_for_each_design: Dict[DesignOfExperiment, np.ndarray] = None,
             estimations_of_parameter_for_each_design: Dict[DesignOfExperiment, np.ndarray] = None,
        ...
        number_of_parameters = len(
            self.calculate(evaluations_blackbox_function=evaluations_blackbox_function_for_each_design[design],
                           estimations_of_parameter=estimations_of_parameter_for_each_design[design],
                           design=design))
        x_dots = [design.name for design in evaluations_blackbox_function_for_each_design.keys()]

        for index in range(number_of_parameters):
            y_dots = [
                self.calculate(evaluations_blackbox_function=evaluations_blackbox_function_for_each_design[design],
                               estimations_of_parameter=estimations_of_parameter_for_each_design[design],
                               design=design)[index] for
                design
                ...
                dict(
                    buttons=list([
                        dict(label=parameter,
                             method="update",
                             args=[
                                 {"visible": [(2 * parameter == other_parameter or 2 * parameter + 1 == other_parameter)
                                              for other_parameter in
                                              range(2 * number_of_parameters)]},
